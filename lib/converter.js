...
var processPugFiles = function(logger, projectDir, successCallback, errorCallback) {
    var pugFilesPath = path.join(projectDir, 'src/app/**/*.pug');
...
function convert(logger, projectDir, options) {
	return new Promise(function (resolve, reject) {
        ...
        if (options.watch) {
            chokidar.watch('src/app/**/*.pug', {
                persistent: true,
                ignoreInitial: true,
                followSymlinks: false,
                ignored: /(^|[\/\\])\../,
            })
            .on('add', onPugFileChange(logger, projectDir))
            .on('change', onPugFileChange(logger, projectDir));
        }
	});
}
