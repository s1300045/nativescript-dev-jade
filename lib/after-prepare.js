...
module.exports = function (logger, platformsData, projectData, hookArgs, $usbLiveSyncService) {
	...
	if (hookArgs.filesToSync !== undefined) {
        var filesToRemove = [];
		hookArgs.filesToSync.forEach(function () {
        ...
        filesToRemove.forEach(function(file) {
            var index = hookArgs.filesToSync.indexOf(index);
            if (index === -1) {
                return;
            }
	...
	return new Promise(function(resolve, reject) {
		...
		var pugFilesPath = path.join(platformData.appDestinationDirectoryPath, 'src/app/**/*.pug');
